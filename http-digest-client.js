...
var HTTPDigest = function () {
  ...
  HTTPDigest.prototype.request = function (options, callback) {
    ...
    options.uri=options.host + options.path;
    request(options,function(error,response,body){

     //console.log("error: " + error);
     // console.log("response:" + JSON.stringify(response.headers));
      //console.log("body: " + body);
      self._handleResponse(options, response, callback);
   ...
   /* request(options, function (res) {
      console.log("options: " + JSON.stringify(options) + " RES1: " + JSON.stringify(res));
  ...
  HTTPDigest.prototype._handleResponse = function handleResponse(options, res, callback) {
    ...
    //console.log("SECOND OPTIONS: " + JSON.stringify(options));

        request(options, function (error,response,body) {
     // console.log("second response: " + JSON.stringify(error) +  " " + JSON.stringify(body));
      callback(body);
    });//.end();
  };
...
module.exports.createDigestClient = function(username, password, https) {
  return new HTTPDigest(username, password, https);
};
