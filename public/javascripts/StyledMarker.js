...
(function() {
  ...
  */
  StyledMarker = function(styledMarkerOptions) {
   ...
   // var ci=me.styleIcon=newstyleicon;
  //  me.bindTo('icon',ci);
 //   me.bindTo('shadow',ci);
  //  me.bindTo('shape',ci);
  //console.log("StyledMArker options: " + JSON.stringify(styledMarkerOptions));
    me.setOptions(styledMarkerOptions);
    
    me.setStyledIcon =function setStyledIcon(si){
        var ci = me.styleIcon =si;
        
           me.bindTo('icon',ci);
            me.bindTo('shadow',ci);
             me.bindTo('shape',ci);
             si.styledmarker=me; //link back
    }
  };
  ...
  */
  StyledIcon = function(styledIconType,styledIconOptions,lat,lon,zindex,styleClass) {
    var k;
    var me=this;
    var i_ = 'icon';
    var sw_ = 'shadow';
    var s_ = 'shape';
    var a_ = [];
   // console.log("create a styledIcon");
    function gs_() {
     ...
     //   console.log("Image width:" + w + "styledmarker" +me.styledmarker);
        ...
        if(me.closemarker==null){
             var closeMarker2 = new google.maps.Marker({icon:new google.maps.MarkerImage("images/simple-dimple-ui-icons-red.png",new google.maps.Size(16,16),new google.maps.Point(32,192),new google.maps.Point(17-w,40)),position:new google.maps.LatLng(lat, lon),draggable:true,raiseOnDrag:false});
       //   console.log("Create a close Marker");
          google.maps.event.addListener(closeMarker2,'click',function(event){simpleDeleteMapPOI(this)});
        
          closeMarker2.setZIndex(zindex);
         if((lat==0)&&(lon==0));
         else closeMarker2.setMap(map);
         me.closemarker=closeMarker2;
        console.log("styled icon, me: " + JSON.stringify(me.styledmarker));
         closeMarker2.bindTo('position', me.styledmarker);
       // closeMarker2.bindTo('visible', me.styledmarker);
            closeMarker2.styledicon=me;
            }
      });
  ...
  */
  ...
  StyledIconTypes.MARKER = {
    ...
    getShadowURL: function(props){
      var rval;
      console.log("getShadowURL: " + JSON.stringify(props));
      if (props.get('starcolor')) {
        rval= bu_ + 'd_map_xpin_shadow&chld=pin_star';
        console.log("getShadowURL returns: " + rval);
        return(rval);
      } else {
        rval= bu_ + 'd_map_pin_shadow';
        console.log("getShadowURL returns: " + rval);
        return(rval);
      }
    },
    ...
    getShape: function(props,width,height){
      ...
      _iconmap.coord = [
       ...
       ((5 / 16) * width), (1 / 8) * height, //leave room for the x
       ( (1 / 2) * width), 0,
  ...
  StyledIconTypes.BUBBLE = {
    ...
    getShadowURL: function(props){
   // console.log( bu_ + 'd_bubble_text_small_shadow&chld=bb|' + props.get('text'));
    return(null);
    },
    getAnchor: function(props,width,height){
